...
public class ArrowImpactHandler : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        Debug.Log($"Arrow hit {collision.collider.name} on layer {collision.collider.gameObject.layer} tag {collision.collider.tag}");

        if (collision.collider.CompareTag("Sky")) {
            ConstellationManager.Instance.PlaceConstellation(collision.GetContact(0).point);
            Destroy(gameObject);
            return;
        }
    ...
    private void HandleStick(Collision collision)
    {
        ...
        // tell circle it has been hit
        if (collision.collider.TryGetComponent(out CircleTarget circle))
            circle.Activate();
